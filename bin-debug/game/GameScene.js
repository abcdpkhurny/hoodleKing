...
var GameScene = (function (_super) {
    ...
    GameScene.prototype.toPlay = function () {
        ...
        var blist = this.barrier.updateBarrier(this.scores, this.prepareBalllist.length, this.round);
        // console.log(blist)
    ...
    /**计算成绩 */
    GameScene.prototype.sumScores = function () {
    ...
    GameScene.prototype.createBall = function () {
        ...
        //玩家弹珠
        var rs = 16;
        var circleShape = new p2.Circle({ radius: rs });
        //可以为任何形状
    ...
    /**开始游戏 */
    GameScene.prototype.runBall = function (e) {
        var _this = this;
        var world = this.world.p2World;
        ...
        var oneball = true;
        //这里用定时器
        this.sendBallTime = window.setInterval(function () {
            if (_this.prepareBalllist.length > 0) {
                var ball = _this.prepareBalllist.shift();
                //添加到发球组
                _this.ballBodylist.push(ball);
                if (!oneball) {
                    ball.position = [GameConst.StageW / 2, 220];
                    world.addBody(ball);
                    _this.addChild(ball.displays[0]);
                }
                oneball = false;
                ball.velocity = [Math.cos(angle) * speed, Math.sin(angle) * speed];
            }
            if (_this.prepareBalllist.length == 0) {
                _this.world.p2World.gravity = [0, 10 * _this.factor];
            }
        }, 200);
    };
    /**循环处理 */
    GameScene.prototype.loopBackFun = function () {
        var _this = this;
        this.sumScores();
        //球吸收完毕
            ...
            if (this.prepareBalllist.length == 0) {
                ...
                if (flay) {
                    ...
                    for (var i = 0; i < this.barrierlist.length; i++) {
                        ...
                        if (black["skin"])
                            egret.Tween.get(black["skin"].txt).to({ y: black["skin"].txt.y - upY }, 500);
                    }
                    window.setTimeout(function () {
                        var blist = _this.barrier.updateBarrier(_this.scores, _this.recyclelist.length, _this.round);
                        _this.addBarrier(blist);
                        _this.addBarrierMaterial(blist, _this.recyclelist);
                        _this.rotateover();
                    }, 500);
                }
            }
    ...
    /**打开排行榜*/
    GameScene.prototype.openRanking = function () {
        var _this = this;
        if (!this.rankingUI)
        ...
        if (this.scores > GameConst.player.score) {
            var url = GameConst.url + "/hoodlePro/updateRankingScore.do";
            var param = "openid=" + GameConst.player.openid + "&score=" + this.scores;
        ...
        this.rankingUI.mbtnRanking.once(egret.TouchEvent.TOUCH_TAP, function () {
            ...
            _this.rankingNom.initParent(_this);
            _this.rankingNom.hasRestart = false;
            _this.rankingNom.curScore = _this.scores;
    ...
    //检验
    GameScene.prototype.openCheckUI = function () {
        var _this = this;
        if (!this.checkUI) {
        ...
        this.checkUI.once(egret.Event.REMOVED_FROM_STAGE, function () {
            ...
            if (_this.checkUI.isExit) {
                ...
                if (_this.oneOpenFlay) {
                    _this.oneOpenFlay = false;
                    _this.playInfo = new PlayInfo();
                    _this.addChild(_this.playInfo);
                    _this.playInfo.init();
                    _this.playInfo.addEventListener(egret.Event.REMOVED_FROM_STAGE, function () {
                        _this.world.stage.once(egret.TouchEvent.TOUCH_END, _this.runBall, _this);
                    }, _this);
                }
                else {
                    //一键发球
    ...
    /**清除所有定时器 */
    GameScene.prototype.clearTime = function () {
        window.clearInterval(this.sendBallTime);
    };
    //一轮结束初始化
    GameScene.prototype.rotateover = function () {
    ...
    /**刚体碰撞处理 */
    GameScene.prototype.preSolveEquation = function () {
        var that = this;
        var world = this.world;
    ...
    /**删除数组中的对象 */
    GameScene.prototype.removeObjList = function (list, obj) {
    ...
    GameScene.prototype.removeP2Body = function (body) {
        var world = this.world;
        world.p2World.removeBody(body);
        if (body.displays[0])
            GameConst.removeChild(body.displays[0]);
        if (body["skin"])
            GameConst.removeChild(body["skin"].txt);
    };
    /**初始化球体 */
    GameScene.prototype.initClearBall = function (ball) {
    ...
    /**添加到障碍物数组 */
    GameScene.prototype.addBarrier = function (list) {
    ...
    GameScene.prototype.addBarrierMaterial = function (barrierlist, balllist, type) {
        ...
        for (var i = 0; i < barrierlist.length; i++) {
            ...
            for (var j = 0; j < balllist.length; j++) {
                var ball = balllist[j];
                if (type == 0)
                    this.setMaterial(barrier, ball);
                else if (type == 1)
                    this.setMaterial(ball, barrier);
            }
        }
    };
    /**3墙实现完全弹性碰撞 */
    GameScene.prototype.addBallMaterial = function (ball) {
        var world = this.world.p2World;
    ...
    GameScene.prototype.setMaterial = function (body1, body2) {
        ...
        var roleAndStoneMaterial = new p2.ContactMaterial(materialA, body2.shapes[0].material); //弹性，摩擦力
